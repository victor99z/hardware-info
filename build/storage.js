...
function dropTable() {
    ...
    db.run(query, (err) => {
        if (err) {
            logger_1.default.error("Error dropping table:", err);
        }
        else {
            logger_1.default.info("Table dropped successfully");
        }
    });
}
function createTable() {
    ...
    db.run(query, (err) => {
        if (err) {
            logger_1.default.error("Error creating table:", err);
        }
        else {
...
function insertData(price, title, loja, url) {
    const db = new sqlite3_1.default.Database("sqlite3.db");
    const query = `INSERT INTO historico (price, title, loja, url) VALUES (?, ?, ?, ?)`;
    db.run(query, [price, title, loja, url], (err) => {
        if (err) {
            logger_1.default.error("Error inserting data:", err);
        }
        else {
            logger_1.default.info("Data inserted successfully");
        }
    });
    db.close();
}
