    ...
    for (const item of items) {
      while (query_page <= 4) {
        ...
        try {
          ...
          // let query_url = item.url + `?page=${query_page}`;
          await page.goto(item.url + "?page=" + query_page, {
          ...
          try {
            ...
            for (const productElement of productElements) {
              ...
              // mui-8rpawh-out_of_stock
              const isOutOfStock = await productElement.$(
                'p[class*="out_of_stock"]'
              );
              if (isOutOfStock) {
                query_page = -1;
                log.info("Product is out of stock, skipping...");
                break; // Skip to the next product
              }
